...
#define DEVICE_NAME                     "SmartShowS2"                               /**< Name of device. Will be included in the advertising data. */
...
#define BLE_Tx_POWER								    0                                         /* [in] tx_power Radio transmit power in dBm (accepted values are -40, -30, -20, -16, -12, -8, -4, 0, and 4 dBm).  */
...
void uart_delay_send_handle();
void uart_reinit(void);
void uart_power_off(void);
...
static void on_ble_evt(ble_evt_t * p_ble_evt)
{
    ...
    switch (p_ble_evt->header.evt_id)
    {
        case BLE_GAP_EVT_CONNECTED:
            ...
            //nrf_gpio_pin_clear(ADVERTISING_LED_PIN_NO);
						write_ble_status(BLE_CONNECTED);
...
{
    ble_conn_params_on_ble_evt(p_ble_evt);//函数用于处理应用程序的堆栈BLE事件。处理来自BLE栈感兴趣的这个模块的所有事件
//p_ble_evt从BLE堆栈接收到的事件。
...
/**@brief   Function for the stop connect.
 *
 * @details 
 */
void ble_stop_current_connect(void * p_event_data, uint16_t event_size)
{
	int tt=0;
	 if(m_conn_handle!=BLE_CONN_HANDLE_INVALID)
	 {
			tt=sd_ble_gap_disconnect(m_conn_handle, BLE_HCI_REMOTE_USER_TERMINATED_CONNECTION);
			ble_log("ble_stop_current_connect...");
	 }
}
...
static void uart_init(void)
{
    ...
    /**@snippet [UART Initialization] */
		task_set(UART_TASK,uart_delay_send_handle);
		task_set(UART_POWER_DOWN_TASK,uart_power_off);
}
/**@brief  Function for initializing the UART module.
 */
void uart_reinit(void)
{
	if(!(NRF_UART0->POWER & UART_POWER_POWER_Msk))
	{		
	...
	task_restart(UART_POWER_DOWN_TASK,10);

}
void uart_power_off(void)
{
	extern int get_uart_enable_pin_state();
	if(get_uart_enable_pin_state()==1)
	{
		//ble_log("uart_power_off...");
...
void UART0_IRQHandler(void)
{
    ...
    index++;
		...
		else
		{
			...
			{
					uint32_t err_code = ble_nus_send_string(&m_nus, data_array, index);
