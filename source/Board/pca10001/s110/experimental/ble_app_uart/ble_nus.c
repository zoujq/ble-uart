...
static void on_write(ble_nus_t * p_nus, ble_evt_t * p_ble_evt)
{
    ...
    if (
        (p_evt_write->handle == p_nus->rx_tx_handles.cccd_handle)
        &&
        (p_evt_write->len == 2)
       )
    {
        ...
        else
        {
            p_nus->is_notification_enabled = false;
             ...
             (p_evt_write->handle == p_nus->rx_tx_handles.value_handle)
             &&
...
/**@brief       Function for adding RX_TX characteristic.
 *
 * @param[in]   p_nus        Nordic UART Service structure.
 ...
 */
static uint32_t rx_tx_char_add(ble_nus_t * p_nus, const ble_nus_init_t * p_nus_init)
{
    ...
    char_md.char_props.notify = 1;
	  ...
	  char_md.char_props.write_wo_resp    = 1;
		char_md.char_user_desc_max_size=50;
		char_md.p_char_user_desc  = "Ble Uart";
    ...
    ble_uuid.uuid             = BLE_UUID_NUS_RX_TX_CHARACTERISTIC;
...
//    }
    ...
    err_code = rx_tx_char_add(p_nus, p_nus_init);
    if (err_code != NRF_SUCCESS)
    {
        return err_code;
    }
...
uint32_t ble_nus_send_string(ble_nus_t * p_nus, uint8_t * string, uint16_t length)
{
    ...
    hvx_params.handle = p_nus->rx_tx_handles.value_handle;
