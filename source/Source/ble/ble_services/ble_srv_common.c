                                  ...
                                  const ble_srv_report_ref_t * p_report_ref)
{
    ...
    p_encoded_buffer[len++] = p_report_ref->report_id;//报告id
    p_encoded_buffer[len++] = p_report_ref->report_type;//报告类型
    
    APP_ERROR_CHECK_BOOL(len == BLE_SRV_ENCODED_REPORT_REF_LEN);//宏调用错误处理功能，如果提供的布尔值是假//
    return len;
}
